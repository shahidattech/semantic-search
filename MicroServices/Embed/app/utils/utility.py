...
def save_files_on_disk(user_id: str, tenant_id:str, files: List):
    try:
        ...
        for file in files:
            ...
            file_path = f"{FOLDER_BASE}/{str(user_id)}/{str(tenant_id)}/"
            if not os.path.exists(file_path):
                os.makedirs(file_path)
            final_file_file_path = f"{file_path }{gen_file_name}"
            with open(final_file_file_path, 'wb') as file_obj:
                file_obj.write(file.file.read())
                file_list.append(final_file_file_path)
